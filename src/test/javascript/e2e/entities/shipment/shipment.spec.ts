...
describe('Shipment e2e test', () => {
  ...
  it('should load Shipments', async () => {
    ...
    expect(await shipmentComponentsPage.getTitle()).to.eq('Shipments');
  });
  ...
  it('should load create Shipment page', async () => {
    ...
    expect(await shipmentUpdatePage.getPageTitle()).to.eq('Create or edit a Shipment');
  ...
  /* it('should delete last Shipment', async () => {
        ...
        expect(await shipmentDeleteDialog.getDialogTitle())
            .to.eq('Are you sure you want to delete this Shipment?');
